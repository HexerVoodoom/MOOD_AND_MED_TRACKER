          ...
          {medications.map((med) => (
            ...
            >
              <div className="flex items-start gap-4 mb-4">
                {/* Pill Visualization */}
                {med.pill && (
                  <div className="w-16 h-16 rounded-xl bg-white border border-[rgb(var(--color-border))] flex items-center justify-center flex-shrink-0 overflow-hidden">
                    {med.pill.image ? (
                      <img src={med.pill.image} alt={med.name} className="w-full h-full object-cover" />
                    ) : (
                      <PillVisualization 
